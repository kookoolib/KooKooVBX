...
class VBX_Incoming_numbers extends Model
{
	...
	function get_numbers($retrieve_sandbox = true)
	{
		...
		try {
			...
			/** Twilio **/
			//KooKoo modifying for KooKoo REST API.
			$items = array();
			if ($this->twilio_sid /*&& $this->twilio_token*/) {
				$nextpageuri = "api/userinfo/numbers/api_key/{$this->twilio_sid}";
					/* Get IncomingNumbers */
					try
					{
					...
					{
						throw new VBX_IncomingNumberException('Failed to connect to KooKoo.', 503);
					}
					...
					if($response->IsError)
					{
						...
						$phoneNumbers = $response->ResponseXml->assigned_number
							? $response->ResponseXml->assigned_number
							: array($response->ResponseXml->assigned_number);
		...
		$ci = &get_instance();
		//KooKoo-Disabling Sandbox number in KooKoo
	...
	function assign_flow($phone_id, $flow_id)
	{
		if (strpos($phone_id, 'tropo-') === 0) {
			...
			if(strtolower($phone_id) == 'sandbox')
				...
				//Updating for KooKoo REST
				//$rest_url = "Accounts/{$this->twilio_sid}/IncomingPhoneNumbers/$phone_id";
				$rest_url="api/numbersapi/update_kookoonumber";
				$kookoo_key="{$this->twilio_sid}";
			$response = $this->twilio->request($rest_url,
											   'POST',
											   array('apikey' => $kookoo_key,
													 'number' => $phone_id,
													 'app_url' => $voice_url,
													 )
											   );
